...
- Built-in mutation sandboxes now run `forge build` once before the mutant loop
...
- Parser-only Call Hierarchy preparation now uses strict bare-symbol import
  exposure, preventing namespace-only free functions from surfacing when the
  cursor is on an unresolved bare call.
...
- Imported free functions bound with `using {fn as alias} for T` now resolve
  through the exposed alias across hover, definition, signature help,
  completion, call hierarchy, and Project Graph call edges.
...
- Project Graph state-variable nodes now carry visibility/getter metadata, and
